...
def get_total_CN(cov_file):
    ...
    av_por_cov = float(all_reg[0][3])/(float(all_reg[0][2]) - float(all_reg[0][1]))
    ...
    av_5pr_e2 = float(all_reg[3][3])/(float(all_reg[3][2]) - float(all_reg[3][1]))
    av_e2_e13 = float(all_reg[4][3])/(float(all_reg[4][2]) - float(all_reg[4][1]))

    av_ctrl_cov = (av_vdr_cov + av_egfr_cov)/2

    comp_av = av_por_cov/av_ctrl_cov
    temp_cn = 2 * comp_av
    total_cn = round(temp_cn)


    return [str(int(total_cn)), round(av_por_cov), round(av_ctrl_cov), str(av_5pr_e2), str(av_e2_e13)];
...
def del_41_test1(cov_5pr_e2, cov_e2_e13):

    if 0.85 < float(cov_e2_e13)/float(cov_5pr_e2) < 1.2:
        return 'norm_var'

    elif 0.25 < float(cov_e2_e13)/float(cov_5pr_e2) < 0.75:
        return 'del_41'

    elif float(cov_e2_e13)/float(cov_5pr_e2) < 0.25:
        return 'del_41_2'
